...
def send_room_message(sid, message):
    sio.emit('eletropunq', {'data': message['data'], 'sid':sid, 'counter':message['counter']}, room=message['room'],
             namespace='/test')
...
@sio.on('timer', namespace='/test')
def send_room_message(sid, message):
    sio.emit('timer', {'data': message['data'], 'sid':sid},namespace='/test')
